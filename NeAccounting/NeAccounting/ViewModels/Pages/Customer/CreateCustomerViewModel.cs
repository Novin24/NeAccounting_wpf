...
namespace NeAccounting.ViewModels
{
    public partial class CreateCustomerViewModel(ISnackbarService snackbarService, INavigationService navigationService) : ObservableObject
    {
        ...
        private async Task OnCreateCustomer()
        {
            ...
            {
                var (error, isSuccess) = await db.customerManager.CreateCustomer(FullName, Mobile, CashCredit.Value + PromissoryNote.Value, CashCredit.Value, PromissoryNote.Value, NationalCode, Address, (CustomerType)CusType, HavePromissoryNote, HaveCashCredit, Buyer, Seller);
                if (!isSuccess)
                {
