...
public partial class CreatePayChequeViewModel(ISnackbarService snackbarService, INavigationService navigationService) : ObservableObject, INavigationAware
{
    ...
    private async Task OnSubmit()
    {
        ...
        using UnitOfWork db = new();
        var (e, s) = await db.DocumentManager.CreateCheque(CusId.Value, Status, ChequeStatus.Payed, Description, SubmitDate.Value, DueDate.Value, Price.Value, false, Cheque_Number, Accunt_Number, Bank_Name, Bank_Branch, Cheque_Owner);
        if (s)
        {
